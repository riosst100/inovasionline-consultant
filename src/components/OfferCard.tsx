...
import { respondToOffer } from "@/lib/actions/projects";
...
export default function OfferCard({
  ...
  const [state, formAction, pending] = useActionState(respondToOffer, undefined);
          ...
          {offer.negotiations.map((n) => (
            <div
              ...
              className={`rounded-xl p-4 text-sm ${
                n.sender === "ADMIN" ? "bg-blue-50 border border-blue-100" : "bg-white border border-slate-200 ml-6"
              }`}
            >
              <p className="text-xs font-semibold text-slate-500 mb-1">
                {n.sender === "ADMIN" ? t.dashboard.senderAdmin : t.dashboard.senderYou} &middot;{" "}
          ...
          {!showNegotiate ? (
            <div className="flex flex-wrap gap-3">
              <form action={formAction}>
                ...
                <button
                  type="submit"
                  disabled={pending}
                  className="bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 text-white font-semibold text-sm px-5 py-2.5 rounded-full transition"
                >
              ...
              <button
                ...
                className="bg-blue-50 hover:bg-blue-100 text-blue-700 font-semibold text-sm px-5 py-2.5 rounded-full transition"
              >
                {t.dashboard.proposeNegotiation}
              </button>
              <form action={formAction}>
                <input type="hidden" name="offerId" value={offer.id} />
                <input type="hidden" name="projectId" value={projectId} />
                ...
                <button
                  type="submit"
                  disabled={pending}
                  className="bg-red-50 hover:bg-red-100 disabled:opacity-60 text-red-600 font-semibold text-sm px-5 py-2.5 rounded-full transition"
                >
                  {t.dashboard.reject}
                </button>
              </form>
            ...
            <form action={formAction} className="space-y-3">
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1.5">{t.dashboard.proposedBudget}</label>
                  <input name="proposedBudget" type="text" className="form-input" placeholder={t.dashboard.proposedBudgetPlaceholder} />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1.5">{t.dashboard.proposedTime}</label>
                  <input name="proposedWaktu" type="text" className="form-input" placeholder={t.dashboard.proposedTimePlaceholder} />
                </div>
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1.5">{t.dashboard.negotiationMessage} <Required /></label>
                <textarea name="message" required rows={3} className="form-input" placeholder={t.dashboard.negotiationMessagePlaceholder} />
              </div>
              ...
              <div className="flex gap-3">
                ...
                >
                  {pending ? t.dashboard.sending : t.dashboard.sendNegotiation}
                </button>
                <button
                  type="button"
