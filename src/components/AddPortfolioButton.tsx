...
export default function AddPortfolioButton({ t }: { t: Dictionary }) {
    ...
    return (
      <div className="bg-white rounded-2xl border border-blue-200 p-6 mb-4">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-bold text-slate-900">{t.dashboard.addPortfolioTitle}</h3>
          <button onClick={() => setOpen(false)} className="text-sm text-slate-500 hover:text-slate-700">
            {t.dashboard.cancel}
          </button>
        </div>
        <PortfolioForm t={t} onDone={() => setOpen(false)} />
      </div>
    );
  }
